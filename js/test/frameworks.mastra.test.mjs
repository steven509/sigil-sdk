...
import { createSigilMastraExporter, SigilMastraExporter } from '../.test-dist/frameworks/mastra/index.js';
...
test('mastra exporter maps a model generation with agent context', async () => {
  ...
  const mastraExporter = createSigilMastraExporter(client, { agentVersion: '1.2.3' });
  ...
  assert.equal(generation.tags['sigil.framework.source'], 'exporter');
  assert.equal(generation.tags['sigil.framework.language'], 'typescript');
  assert.equal(generation.metadata['sigil.framework.run_id'], GEN_SPAN_ID);
  assert.equal(generation.metadata['sigil.framework.parent_run_id'], AGENT_SPAN_ID);
  assert.equal(generation.metadata['sigil.framework.thread_id'], 'thread-42');
  assert.equal(generation.metadata['sigil.framework.run_type'], 'llm');
  assert.equal(generation.metadata['sigil.framework.component_name'], 'Test Agent');
  assert.equal(generation.metadata['sigil.framework.mastra.span_type'], 'model_generation');
  assert.equal(generation.metadata['sigil.framework.mastra.provider'], 'openai.chat');
  assert.equal(generation.metadata['sigil.framework.mastra.metadata'].threadId, 'thread-42');
...
test('mastra exporter exports workflow steps with links and chained parents', async () => {
  ...
  assert.equal(step1.conversationId, `sigil:framework:mastra:${workflowTraceId}`);
  ...
  assert.equal(step1.metadata['sigil.framework.mastra.workflow'], 'my-workflow');
  assert.equal(step1.metadata['sigil.framework.mastra.status'], 'success');

  assert.equal(step1.linkedGenerationIds.length, 1);
  const generation = exporter.generations[0];
  assert.equal(step1.linkedGenerationIds[0], generation.id);
  assert.equal(generation.conversationId, `sigil:framework:mastra:${workflowTraceId}`);
  assert.deepEqual(generation.metadata['sigil.framework.tags'], ['team-a']);
...
test('mastra exporter maps errors onto generations', async () => {
  ...
  assert.equal(exporter.generations[0].metadata['sigil.framework.mastra.error.id'], 'RATE_LIMIT');
  assert.equal(exporter.generations[0].metadata['sigil.framework.mastra.error.category'], 'RATE_LIMIT');
});
...
test('mastra generation spans join the mastra trace with span lineage', async () => {
  ...
  const tracer = tracerProvider.getTracer('sigil-mastra-test');
...
test('mastra exporter survives hostile metadata without dropping the generation', async () => {
  ...
  assert.equal(metadata['sigil.framework.mastra.metadata'].threadId, 'thread-42');
  assert.equal(metadata['sigil.framework.mastra.metadata'].alsoBad, undefined);
});
...
test('sigil client debug buffers are bounded', async () => {
  const { client } = newClient();
...
test('mastra exporter surfaces rolled-up usage from internal model spans', async () => {
  ...
  assert.equal(rollup.metadata['sigil.framework.mastra.usage_rollup'], true);
});
