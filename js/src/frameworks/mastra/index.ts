...
import {
  ...
  type SigilMastraExporterOptions,
} from './types.js';
...
const metadataKeyRunID = 'sigil.framework.run_id';
const metadataKeyRunType = 'sigil.framework.run_type';
const metadataKeyThreadID = 'sigil.framework.thread_id';
const metadataKeyParentRunID = 'sigil.framework.parent_run_id';
const metadataKeyComponentName = 'sigil.framework.component_name';
const metadataKeyTags = 'sigil.framework.tags';
const metadataKeyMastraSpanType = 'sigil.framework.mastra.span_type';
const metadataKeyMastraProvider = 'sigil.framework.mastra.provider';
const metadataKeyMastraMetadata = 'sigil.framework.mastra.metadata';
const metadataKeyMastraUsageRollup = 'sigil.framework.mastra.usage_rollup';
...
/**
 * Mastra observability exporter backed by a {@link SigilClient}.
 *
 ...
 *
 * const sigil = new SigilClient(); // reads SIGIL_* env vars
 ...
 *
 * Mapping:
 * - `model_generation` spans become Sigil generations (plus `gen_ai.client.*`
 ...
 * - `workflow_step` spans become Sigil workflow steps with linked generations
 ...
 *
 * OTel spans created by the Sigil client are parented on the originating
 ...
 */
export class SigilMastraExporter implements MastraObservabilityExporterLike {
  readonly name = 'sigil';

  private readonly client: SigilClient;
  private readonly ownsClient: boolean;
  private readonly options: SigilMastraExporterOptions;
  ...
  constructor(client: SigilClient, options?: SigilMastraExporterOptions);
  constructor(options?: SigilMastraExporterOptions);
  constructor(clientOrOptions?: SigilClient | SigilMastraExporterOptions, maybeOptions?: SigilMastraExporterOptions) {
    if (isSigilClient(clientOrOptions)) {
      this.client = clientOrOptions;
      this.ownsClient = false;
      this.options = { ...(maybeOptions ?? {}) };
    } else {
      this.client = new SigilClient();
  ...
  async exportTracingEvent(event: MastraTracingEvent): Promise<void> {
    try {
      ...
      this.logWarn('sigil mastra exporter failed to process tracing event', error);
    }
  }
  ...
  async flush(): Promise<void> {
    try {
      ...
      this.logWarn('sigil client flush failed', error);
    }
  }

  /** Releases per-trace state; shuts the owned Sigil client down. */
  async shutdown(): Promise<void> {
    ...
    // throw 'sigil client is shutdown'.
    try {
      ...
      this.logWarn('sigil client flush/shutdown during exporter shutdown failed', error);
    }
  }
  ...
  private recordGeneration(state: TraceState, span: MastraExportedSpan): void {
    ...
    if (recorderError !== undefined) {
      this.logWarn('sigil mastra exporter failed to record generation', recorderError);
      return;
    }
   ...
   * silently zero every token count in Sigil. `internalUsage` never overlaps
  ...
  private recordUsageRollup(state: TraceState, span: MastraExportedSpan): void {
    ...
    const recorderError = recorder.getError();
    if (recorderError !== undefined) {
      this.logWarn('sigil mastra exporter failed to record usage rollup', recorderError);
      return;
    }
    state.lastGenerationId = generationId;
  }
  ...
  private recordWorkflowStep(state: TraceState, span: MastraExportedSpan): void {
    ...
    if (status !== undefined) {
      metadata['sigil.framework.mastra.status'] = status;
    }
    const errorClassification = mapMastraErrorClassification(span.errorInfo);
    if (errorClassification !== undefined) {
      Object.assign(metadata, errorClassification);
    ...
    if (workflowName !== undefined) {
      metadata['sigil.framework.mastra.workflow'] = workflowName;
    }
  ...
  private resolveConversationId(state: TraceState, span: MastraExportedSpan): string {
    ...
    return `sigil:framework:${frameworkName}:${span.traceId}`;
  }
   ...
   * Sigil-created span joins the Mastra trace. When the Mastra spans are also
   * exported over OTLP (e.g. via `@mastra/otel-exporter`, which preserves
   * Mastra's OTel-compatible ids), the Sigil span nests under the Mastra span
   * in the same trace.
   *
  ...
  private logWarn(message: string, error?: unknown): void {
    if (this.mastraLogger?.warn !== undefined) {
      this.mastraLogger.warn(`[SigilMastraExporter] ${message}`, error);
      return;
    }
    ...
    if (typeof internalClient.internalLogWarn === 'function') {
      internalClient.internalLogWarn(`[SigilMastraExporter] ${message}`, error);
    }
  }
}

/**
 * Creates a {@link SigilMastraExporter}.
 *
 * Pass an existing {@link SigilClient} to share transport/config with other
 * instrumentation, or omit it to let the exporter construct one from `SIGIL_*`
 * environment variables (the exporter then owns the client and shuts it down
 * on `shutdown()`).
 */
export function createSigilMastraExporter(
  client: SigilClient,
  options?: SigilMastraExporterOptions,
): SigilMastraExporter;
export function createSigilMastraExporter(options?: SigilMastraExporterOptions): SigilMastraExporter;
export function createSigilMastraExporter(
  clientOrOptions?: SigilClient | SigilMastraExporterOptions,
  maybeOptions?: SigilMastraExporterOptions,
): SigilMastraExporter {
  if (isSigilClient(clientOrOptions)) {
    return new SigilMastraExporter(clientOrOptions, maybeOptions);
  }
  return new SigilMastraExporter(clientOrOptions);
}
