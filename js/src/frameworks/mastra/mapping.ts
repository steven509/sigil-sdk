...
/**
 * Resolves an Agento11y provider name from Mastra's provider string (an AI SDK
...
/** Maps Mastra `UsageStats` to Agento11y {@link TokenUsage}. */
...
function mapOneMessage(raw: unknown, defaultRole: string): Message | undefined {
  ...
  if (Array.isArray(content)) {
    ...
    if (parts.length > 0) {
      // Agento11y only accepts tool_result parts on tool-role messages; some
 ...
 * plain prompt string) to Agento11y messages. The AI SDK v5 message shape
 ...
 * message list, or a plain string) to Agento11y assistant messages.
 */
...
/**
 * Splits system messages out of a mapped input list; Agento11y carries the system
 ...
 * domain`) so it survives as metadata beside the Agento11y-derived
...
export function mapMastraErrorClassification(errorInfo: unknown): Record<string, string> | undefined {
  ...
  if (id !== undefined) {
    classification['agento11y.framework.mastra.error.id'] = id;
  }
  const category = asStringOrUndefined(record.category);
  if (category !== undefined) {
    classification['agento11y.framework.mastra.error.category'] = category;
  }
  const domain = asStringOrUndefined(record.domain);
  if (domain !== undefined) {
    classification['agento11y.framework.mastra.error.domain'] = domain;
  }
 ...
 * { type: 'tool'; toolName }`) to Agento11y's string field.
 */
...
/** Maps `agent_run` `availableTools` names to Agento11y tool definitions. */
...
/** Wraps a non-record value so it fits Agento11y's workflow-step state fields. */
